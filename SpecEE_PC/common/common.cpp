...
struct common_init_result common_init_from_params(common_params & params) {
    ...
    if (params.warmup) {
        ...
        if (llama_model_has_decoder(model)) {
            struct llama_batch batch = llama_batch_get_one(tmp.data(), std::min(tmp.size(), (size_t) params.n_batch));
            batch.n_candidates = -1; // 用于区分是否是warmup阶段
...
void batch_add_candidates(
                 struct llama_batch & batch,
                 llama_token_data *llama_token_data_array,
                 int n_candidates) {
    batch.n_candidates    = n_candidates;
    
    std::vector<int32_t> candidate_tokens;
    std::vector<float> candidate_p;
    std::vector<float> candidate_logits;

    for (int k = 0; k < n_candidates; ++k) {
        candidate_tokens.push_back(llama_token_data_array[k].id);
        candidate_p.push_back(llama_token_data_array[k].p);
        candidate_logits.push_back(llama_token_data_array[k].p);
    }

    batch.candidate_tokens = new int32_t[n_candidates];
    batch.candidate_p = new float[n_candidates];
    batch.candidate_logits = new float[n_candidates];

    std::copy(candidate_tokens.begin(), candidate_tokens.end(), batch.candidate_tokens);
    std::copy(candidate_p.begin(), candidate_p.end(), batch.candidate_p);
    std::copy(candidate_logits.begin(), candidate_logits.end(), batch.candidate_logits);
}
//
// Vocab utils
//
