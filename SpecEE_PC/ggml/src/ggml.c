...
struct ggml_tensor * ggml_top_k(
    ...
    // int64_t mne[4] = {k, a->ne[1], a->ne[2], a->ne[3]};
    struct ggml_tensor * result = ggml_new_tensor(ctx, GGML_TYPE_I32, GGML_MAX_DIMS, a->ne);

    ggml_set_op_params_i32(result, 0, k);

    result->op     = GGML_OP_ARGTOPK;
    result->src[0] = a;
...
static void ggml_compute_forward_mul_mat(
    ...
    // if (strncmp(dst->name, "kq-", 3) == 0) {
    //     printf("\n|%s| data:\n", src0->name);
    //     for (int j = 0; j < src0->ne[1]; ++j) {
    //         for (int i = 0; i < src0->ne[0]; ++i) {
    //             printf(" %.2f", *((float*)((char*)src0->data+i*src0->nb[0]+j*src0->nb[1])));
    //         }
    //         printf("\n");
    //     }
    //     printf("\n|%s| data:\n", src1->name);
    //     for (int i = 0; i < src1->ne[0]; ++i) {
    //         printf(" %.2f", *((float*)((char*)src1->data+i*src1->nb[0])));
    //     }
    //     printf("\n");
    // }
...
UseGgmlGemm2:;
    ...
    //         printf(" %.2f", *((float*)((char*)dst->data+i*dst->nb[0])));
    //     }
    //     printf("\n");
    // }
}
...
static void ggml_compute_forward_soft_max_f32(
    ...
    // if (strncmp(src0->name, "kq-", 3) == 0) {
    //     printf("\n|%s| data:\n", dst->name);
    //     for (int i = 0; i < dst->ne[0]; ++i) {
    //         printf(" %2.2f", *((float*)((char*)dst->data+i*dst->nb[0])));
    //     }
    //     printf("\n");
    // }
    // if (src1) {
    ...
    //         for (int i = 0; i < src1->ne[0]; ++i) {
    //             printf(" %.2f", *((float*)((char*)src1->data+i*src1->nb[0]+j*src1->nb[1])));
    //         }
    //         printf("\n");
    //     }
    // } else {
    //     printf("| mask is null - softmax\n");
    // }  // dkv debug
}
...
//     }
// }
static void quick_sort_for_argsort_f32(const enum ggml_sort_order order, const float * src_data, int32_t * dst_data, int p, int r) {
    if (p < r) {
        int32_t x = dst_data[r];
        int i = p-1;
        for (int j = p; j != r-1; ++j){
            if ((order == GGML_SORT_ORDER_ASC  && src_data[dst_data[j]] < src_data[x]) ||
                (order == GGML_SORT_ORDER_DESC && src_data[dst_data[j]] > src_data[x])){
                    ++i;
                    int32_t tmp = dst_data[i];
                    dst_data[i] = dst_data[j];
                    dst_data[j] = tmp;
                }
        }
        int32_t tmp = dst_data[i+1];
        dst_data[i+1] = dst_data[r];
        dst_data[r] = tmp;
        quick_sort_for_argsort_f32(order, src_data, dst_data, p, i);
        quick_sort_for_argsort_f32(order, src_data, dst_data, i+2, r);
    }
}
...
static void ggml_compute_forward_argtopk_f32(
    const struct ggml_compute_params * params,
    struct ggml_tensor * dst) {
    ...
    int k = ggml_get_op_params_i32(dst, 0);
    ...
    for (int64_t i = ith; i < nr; i += nth) {
        ...
        for (int64_t j = 0; j < ne00; ++j) {
            int64_t p = k-1;
            for (; p >= 0; --p) {
                if (dst_data[p] == INT_MIN) {
                    continue;
                } else if (src_data[dst_data[p]] > src_data[j]) {
                    break;
                } else if (p < k-1) {
                    dst_data[p+1] = dst_data[p];
                }
            }
            if (p < k-1) {
                dst_data[p+1] = (int32_t)j;
            }
        }
...
static void ggml_compute_forward(struct ggml_compute_params * params, struct ggml_tensor * tensor) {
    ...
    switch (tensor->op) {
        ...
        case GGML_OP_ARGTOPK:
            {
                ggml_compute_forward_argtopk_f32(params, tensor);  // topk
            } break;
