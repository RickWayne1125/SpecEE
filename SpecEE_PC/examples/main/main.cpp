...
static void my_ggml_print_tensor(ggml_tensor * t) {
...
static bool ggml_debug(struct ggml_tensor * t, bool ask, void * user_data) {
    ...
    if (ask) {
        ...
        // if(strncmp(t->name,"candidate_token_ids",19)==0 || strncmp(t->name,"candidate_token_ids",19)==0,strncmp(t->name,"candidate_lm_head",17)==0||strncmp(t->name,"pred-",5)==0){
        //     return true; // Always retrieve data
        // }else{
        //     return false;
        // }
...
static bool ggml_debug_csv(struct ggml_tensor * t, bool ask, void * user_data) {
    ...
    // if (strncmp(t->name, "kq-", 3) == 0 or strncmp(t->name, "kq_", 3) == 0)
    {
        ...
        printf("%s, %s, %s, %s\n",ggml_ne_string(t).c_str(),ggml_op_desc(t),src0?src0->name:"",src1?src1->name:"");
        //         shape,  op,    src0,  src1, 
    }
    return true;
}

static bool ggml_debug_origin(struct ggml_tensor * t, bool ask, void * user_data) {
    bool is_debug = true;
    bool print_tensor = true;
    auto * cb_data = (callback_data *) user_data;
    // if(cb_data==nullptr) return true;
    const struct ggml_tensor * src0 = t->src[0];
    const struct ggml_tensor * src1 = t->src[1];
